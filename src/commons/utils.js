...
export function encryptProject(project, securityToken) {
  ...
  encrypted.sourceConnection.providerOptions = encryptProviderOptions(
    project.sourceConnection.providerOptions,
    securityToken.key
  )
  encrypted.targetConnection.providerOptions = encryptProviderOptions(
    project.targetConnection.providerOptions,
    securityToken.key
  )

  if (encrypted.exportFormat) {
    encrypted.exportFormat.providerOptions = encryptProviderOptions(
...
export function decryptProject(project, securityToken) {
  ...
  decrypted.sourceConnection.providerOptions = decryptProviderOptions(
    decrypted.sourceConnection.providerOptions,
    securityToken.key
  )
  decrypted.targetConnection.providerOptions = decryptProviderOptions(
    decrypted.targetConnection.providerOptions,
    securityToken.key
  )

  if (decrypted.exportFormat) {
    decrypted.exportFormat.providerOptions = decryptProviderOptions(
...
function decryptProviderOptions(providerOptions, secret) {
  const secureString = providerOptions
