 ...
 */
export class VideoAsset extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.childAssets !== prevProps.childAssets) {
      this.addAssetTimelineTags(
        this.props.childAssets,
        this.getVideoPlayerState().duration
      )
    }
  ...
  movePreviousTaggedFrame = () => {
    ...
    const previousFrame = _.reverse(this.props.childAssets).find(
      asset =>
        asset.state === AssetState.Tagged && asset.timestamp < currentTime
    )
  ...
  moveNextExpectedFrame = () => {
    ...
    const frameSkipTime =
      1 / this.props.additionalSettings.videoSettings.frameExtractionRate
    const seekTime = currentTime + frameSkipTime
   ...
   */
  addAssetTimelineTags = (childAssets, videoDuration) => {
    if (!this.props.autoPlay) {
      return
    }

    const assetTimelineTagLines = this.renderTimeline(
      childAssets,
      videoDuration
    )
   ...
   * @param videoDuration - Length (in seconds) of the video
   */
  renderTimeline = (childAssets, videoDuration) => {
    return (
      <div className={"video-timeline-container"}>
        {childAssets.map(childAsset =>
          this.renderChildAssetMarker(childAsset, videoDuration)
        )}
      </div>
    )
  }
  ...
  renderChildAssetMarker = (childAsset, videoDuration) => {
    const className =
      childAsset.state === AssetState.Tagged
        ? "video-timeline-tagged"
        : "video-timeline-visited"
