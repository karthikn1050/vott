...
import { randomIntInRange } from "../../commons/utils"
...
export class TagInput extends React.Component {
  ...
  onEditTag = tag => {
    ...
    const newEditingTag =
      editingTag && editingTag.name === tag.name ? null : tag
    this.setState({
  ...
  renderTagItems = () => {
    ...
    if (query.length) {
      props = props.filter(prop =>
        prop.tag.name.toLowerCase().includes(query.toLowerCase())
      )
    }
  ...
  createTagItemProps = () => {
    ...
    return tags.map(tag => ({
      ...
      isLocked:
        ...
        this.props.lockedTags.findIndex(t => t === tag.name) > -1,
      isBeingEdited:
        this.state.editingTag && this.state.editingTag.name === tag.name,
      isSelected:
        this.state.selectedTag && this.state.selectedTag.name === tag.name,
  ...
  getNewSelectedTag = (tags, previouIndex) => {
  ...
  onAddTagKeyDown = event => {
    if (event.key === "Enter") {
      ...
      if (
        newTag.name.length &&
        !this.state.tags.find(t => t.name === newTag.name)
      ) {
        this.addTag(newTag)
        event.target.value = ""
