...
export class AssetService {
   ...
   */
  static createAssetFromFilePath(
    assetFilePath,
    assetFileName,
    assetIdentifier
  ) {
   ...
   */
  async getUpdatedAssets(tagName, transformer) {
   ...
   */
  updateTagInAssetMetadata(assetMetadata, tagName, transformer) {
    let foundTag = false
    ...
    if (foundTag) {
      ...
      assetMetadata.asset.state = assetMetadata.regions.length
        ? AssetState.Tagged
        : AssetState.Visited
      return true
    }
  ...
  async getRegionsFromTFRecord(asset) {
    ...
    for (let index = 0; index < objectArray.textArray.length; index++) {
      regions.push({
        ...
        boundingBox: {
          ...
          width:
            (objectArray.xmaxArray[index] - objectArray.xminArray[index]) *
            objectArray.width,
          height:
            (objectArray.ymaxArray[index] - objectArray.yminArray[index]) *
            objectArray.height
        },
        points: [
          {
  ...
  async getTFRecordMetadata(asset) {
    const tfrecords = new Buffer(await HtmlFileReader.getAssetArray(asset))
    const reader = new TFRecordsReader(tfrecords)

    const width = reader.getFeature(0, "image/width", FeatureType.Int64)
    const height = reader.getFeature(0, "image/height", FeatureType.Int64)

    const xminArray = reader.getArrayFeature(
      0,
      "image/object/bbox/xmin",
      FeatureType.Float
    )
    const yminArray = reader.getArrayFeature(
      0,
      "image/object/bbox/ymin",
      FeatureType.Float
    )
    const xmaxArray = reader.getArrayFeature(
      0,
      "image/object/bbox/xmax",
      FeatureType.Float
    )
    const ymaxArray = reader.getArrayFeature(
      0,
      "image/object/bbox/ymax",
      FeatureType.Float
    )
    const textArray = reader.getArrayFeature(
      0,
